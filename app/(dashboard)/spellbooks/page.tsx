...
export default async function SpellbooksPage({
  ...
  if (params.search) {
    filteredSpellbooks = filteredSpellbooks.filter(spellbook => 
      spellbook.name.toLowerCase().includes(params.search!.toLowerCase()) ||
      spellbook.description?.toLowerCase().includes(params.search!.toLowerCase()) ||
      spellbook.tags.some(tag => tag.toLowerCase().includes(params.search!.toLowerCase()))
    )
  }
    ...
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h1 className="text-4xl font-bold mb-2">My Spellbooks</h1>
        ...
        <Card>
          <CardContent className="p-6">
            <div className="flex flex-col md:flex-row gap-4">
              <SearchForm defaultValue={params.search} placeholder="Search spellbooks..." />
              <div className="flex gap-2 flex-wrap">
                <Link
                  href={params.search ? `/spellbooks?search=${params.search}&visibility=public` : '/spellbooks?visibility=public'}
                ...
                <Link
                  href={params.search ? `/spellbooks?search=${params.search}&visibility=private` : '/spellbooks?visibility=private'}
                  className={`px-3 py-1 rounded-lg border transition-all text-sm ${
        ...
        <Card>
          <CardContent className="flex flex-col items-center justify-center py-20 text-center">
            ...
            <h3 className="text-2xl font-semibold mb-2">
              {spellbooks.length === 0 ? 'No spellbooks yet' : 'No spellbooks found'}
            </h3>
          ...
          {(params.search || params.visibility) && (
            <div className="flex items-center justify-between">
              <p className="text-sm text-muted-foreground">
                Found {filteredSpellbooks.length} spellbook{filteredSpellbooks.length !== 1 ? 's' : ''}
